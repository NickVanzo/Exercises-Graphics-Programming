...
void GearsApplication::Render()
{
    ...
    m_shaderProgram.SetUniform(m_viewProjMatrix, camera.GetViewProjectionMatrix());
...
void GearsApplication::InitializeShaders()
{
    ...
    m_worldPosition = m_shaderProgram.GetUniformLocation("WorldMatrix");
...
void GearsApplication::DrawGear(const Mesh& mesh, const glm::mat4& worldMatrix, const Color& color)
{
    ...
    m_shaderProgram.SetUniform(m_worldPosition, worldMatrix);
